...
*
**/
class YahooWeather extends GGoRok
{
...
* @param string $unit default = 'f'. Use 'f' for ¡ÆFahrenheit or 'c' for ¡ÆCelsius. **If you use Celsius, all other units will be metric units as well!**
* @access public
*/
    
    
	public function start($where = "", $unit = 'f')
	{
		$this->woeid = $this->tim_GetWoeID($where);
		$this->unit = $unit;
...
*
* @param bool $showunit default = true, shows '¡ÆF' for Fahrenheit or '¡ÆC' for Celsius
...
*
* @param bool $showunit default = true, shows '¡Æ'
...
*/
    ...
    public function getForecastTomorrowHigh2($showunit = true)
    {
        $temperature = $this->weather_forecast[2]->attributes()->high;
        if($showunit) $temperature .= '&#8239;&#176;'.$this->weather_unit[0]->attributes()->temperature;
        
        return $temperature;
    }
    
    public function getForecastTomorrowHigh3($showunit = true)
    {
        $temperature = $this->weather_forecast[3]->attributes()->high;
        if($showunit) $temperature .= '&#8239;&#176;'.$this->weather_unit[0]->attributes()->temperature;
        
        return $temperature;
    }
    
    public function getForecastTomorrowHigh4($showunit = true)
    {
        $temperature = $this->weather_forecast[4]->attributes()->high;
        if($showunit) $temperature .= '&#8239;&#176;'.$this->weather_unit[0]->attributes()->temperature;
...
*/
    ...
    public function getForecastTomorrowConditionCode2()
    {
        
        return (int)$this->weather_forecast[2]->attributes()->code;
    }
    public function getForecastTomorrowConditionCode3()
    {
        
        return (int)$this->weather_forecast[3]->attributes()->code;
    }
    public function getForecastTomorrowConditionCode4()
    {
        
        return (int)$this->weather_forecast[4]->attributes()->code;
    }
...
*/
	...
	function tim_GetWoeID($Where)
	{
		
		$yql_base_url = "http://query.yahooapis.com/v1/public/yql";  
		$yql_query    = "select * from geo.places  where text='".strtoupper($Where)."'";
		...
		//echo print_r($phpObj['query']['results']['place']);
		if(isset($phpObj['query']['results']['place'][0]))
		{
			return $phpObj['query']['results']['place'][0]['woeid'];
		}
		else
		{
			return $phpObj['query']['results']['place']['woeid'];
		}
