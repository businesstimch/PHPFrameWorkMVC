...
class _HTMLs
{
    ...
    {
		global $login, $language;
		//<a class="hoverU" href="">Tour</a><br />
		
		$this->_lang = $language->loadLanguage('footer_menu');
		...
		<div class="w100" id="footer">
			<div class="outline">
				...
				<div class="footerLink">
					...
					<a href="/login/'.($login->isLogIn()?'?ajaxProcess&menu=logout':'').'">	'.($login->isLogIn()?$this->_lang[_lang_]['logout']:$this->_lang[_lang_]['login']).'</a>
					-->
				</div>
				<div class="footerLink">
	...
	{
		global $db,$login,$language;
		
		$lang = $language->loadLanguage('top_menu');
		$lang_Profile = $language->loadLanguage('burugo_profile');
								...
								<div class="topLogin_LINK">
									...
									<div id="topSettings_Container">
										...
										<div id="topSettings_Contents">
											...
											<div class="divider"></div>
											<a class="topSettings_One topSettings_One_Hover block" '.($login->isLogIn() ? 'href="/login/?ajaxProcess&menu=logout"><span>Log Out</span>':'href="/login/"><span>Log In</span>').'</a>
	...
	{
		global $language;
		$lang = $language->loadLanguage('login');
	...
	{
		global $language,$login,$language;
		$lang = $language->loadLanguage('ceo_menu');
	...
	{
		global $login, $language;
		$lang = $language->loadLanguage('burugo_profile');
		return ($login->isLogIn()? '
			<div class="myProfile_Block">
				<div class="myProfile_Pic"><img src="'.($login->_customerInfo[0]['customers_profile_picture'] != '' ? '/Template/Img/cData/'.$login->_customerID.'/profilePic/thumbnail/'.$login->_customerInfo[0]['customers_profile_picture'] : "/Template/Img/no-profile-image.jpg").'" alt="'.$login->_customerInfo[0]['customers_firstname'].' '.$login->_customerInfo[0]['customers_lastname'].'" /></div>
				<div class="myProfile_Link">
					'.$login->_customerInfo[0]['customers_firstname'].' '.$login->_customerInfo[0]['customers_lastname'].'<br />
	...
	function __tag_Tree_wwwHome_left__()
	{
		...
		$this->_lang = $language->loadLanguage('tree_left_menu');
	...
	function __tag_ceoHome_right__()
	{
		...
		return '
				...
				<a href="'.($login->isLogIn()? "http://ceo.".__domain__."/mystore/add-store/" : "http://ceo.".__domain__."/register/").'" class="right_banner"><img src="/Template/Img/business-service-add-'._lang_.'.gif"></a>
	...
	function __tag_cpHome_LeftProfile()
	{
		...
		return ($login->isLogIn()? '
			<div id="myProfileCP_Block">
				<div id="myProfileCP_Pic"><img src="'.($login->_customerInfo[0]['customers_profile_picture'] != '' ? '/Template/Img/cData/'.$login->_customerID.'/profilePic/thumbnail/'.$login->_customerInfo[0]['customers_profile_picture'] : "/Template/Img/no-profile-image.jpg").'" alt="'.$login->_customerInfo[0]['customers_firstname'].' '.$login->_customerInfo[0]['customers_lastname'].'" /></div>
