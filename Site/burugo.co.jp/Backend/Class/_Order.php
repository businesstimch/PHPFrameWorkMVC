	...
	function get_pendingOrder($Owner_customerID,$timestamp_Later = false)
	{
		...
		$LastPending = $db->QRY("
			...
			WHERE
				...
				s.customers_id = '".$db->escape($Owner_customerID)."' AND
			...
			WHERE
				...
				(
					...
					FROM
						b_burugo_stores
					WHERE
						customers_id = '".$db->escape($Owner_customerID)."'
				)
...
//.(!is_null($limit)?" LIMIT $limit":"")."
	...
	function get_StatusTXT($statusID)
	{
		global $language;
		$status ='';
		$lang = $language->loadLanguage('order');
	...
	function get_Orders($customerID, $orderBy='order_date', $pg=1, $limit=20, $onlyPending=false)
	{
		...
		$order['order'] = $db->QRY("
		    ...
		    WHERE
				o.customers_id = '".$db->escape($customerID)."'
	...
	function get_Order_Summary($customerID)
	{
		...
		$orderList = $db->QRY("
			...
			WHERE
				b_burugo_order.customers_id = '".$db->escape($customerID)."' AND
