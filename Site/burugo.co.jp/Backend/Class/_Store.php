...
class _Store {
	...
	function SQL_AllCategory($Domain = null)
	{
		...
		if(!is_null($Domain))
		{
			$WHERE .= " AND c.category_subdomain = '".$db->escape($Domain)."'";
		}
		...
		$C = $db->QRY("
			...
			WHERE
				cN.lang_id = '".$db->escape(__langID__)."'
				".$WHERE."
			ORDER BY
				c.category_sort ASC
		");
	...
	function getCategoryID_by_url($catURL = null)
	{
		...
		$where = "";
		$where .= (!is_null($catURL)? " c.category_url = '".($db->escape($catURL))."'" : "");
			...
			FROM
				b_category c
					LEFT JOIN
						b_category_name cn
					ON
						c.category_id = cn.category_id AND cn.lang_id = '".$db->escape(__langID__)."'
			WHERE
				".$where."
				
				/*
					AND c.category_subdomain = '".$db->escape(_SubDomain_)."'
				*/
		");
		foreach($C as $C_F)
		{
	...
	function getStoreID_by_URL($StoreURL)
	{
		...
		$StoreID = $db->QRY("
			...
			FROM
				b_burugo_stores s
			WHERE
				s.store_url = '".$db->escape($StoreURL)."' AND
				s.customers_id = '".$db->escape($login->_customerID)."'
		");
		if(sizeof($StoreID) > 0)
	...
	{
		global $db,$login,$store;
		
		$where_secure = '';
		if($secure_access)
			$where_secure = ' AND customers_id = "'.$db->escape($login->_customerID).'"';
		$Store = $db->QRY("
			SELECT
				*
			FROM
				b_burugo_stores s
			WHERE
				s.store_id = '".$db->escape($this->getStoreID_by_URL($StoreDomain))."'"
				.$where_secure."
		");
		return $Store;
	}
	...
	function saveStoreCategory($StoreID, $categoryIDs)
	{
		...
		foreach($categoryIDs_Arr as $categoryIDs_Arr_F)
		{
			...
			if(sizeof($db->QRY("SELECT category_id FROM b_category WHERE category_id = '".$categoryIDs_Arr_F."'")) > 0)
			{
				$db->QRY("
					INSERT INTO
						b_category_to_store
	...
	function loadThisStore_by_ID($StoreID,$secure_access = false, $Get_Also_DeactivedStore = false)
	{
		...
		$output_temp = $db->QRY("
			...
			FROM
				b_burugo_stores s
			WHERE
				s.store_id = '".$db->escape($StoreID)."'
				".($Get_Also_DeactivedStore?"":" AND s.store_active = 1 ")."
				".($secure_access?' AND s.customers_id = "'.$db->escape($login->_customerID).'"':'')."
		...
		if(sizeof($output_temp) > 0)
		{
			...
			#[Load] : Store Category
			$output['Store_Categories'] = $db->QRY("
				SELECT
					*
				FROM
					b_category_to_store s2c
				WHERE
					s2c.store_id = '".$db->escape($StoreID)."'
			");
			
			
			#[Load] : Store List Images
			$output['Store_Images'] = $db->QRY("
				SELECT
					*
				FROM
					b_burugo_stores_images i
				WHERE
					i.store_id = '".$db->escape($StoreID)."'"
					.($secure_access?' AND i.customers_id = "'.$db->escape($login->_customerID).'"':'')."
				ORDER BY
			...
			$output['Store_Desc_Images'] = $db->QRY("
				SELECT
					*
				FROM
			...
			$output['Store_Sub_Category'] = $db->QRY("
				SELECT
				...
				WHERE
					...
					sub_c2S.store_id = '".$db->escape($StoreID)."'" 
					.($secure_access?' AND sub_c2S.customers_id = "'.$db->escape($login->_customerID).'"':'')."
			");
			...
			if(sizeof($Subscription_Package) > 0)
			{
				$output_temp = $db->QRY("
					...
					WHERE
						...
						ph.store_id = '".$db->escape($StoreID)."' AND
			...
			*/
			...
			$output['Store_Group'] = $db->QRY("
				...
				WHERE
					store_id = '".$db->escape($StoreID)."'"
					.($secure_access?' AND customers_id = "'.$db->escape($login->_customerID).'"':'')."
			");
			...
			foreach($output_temp AS $ItemGroup)
			{
				$Items = $db->QRY("
					...
					FROM
						b_burugo_store_item
					WHERE
						group_id = '".$db->escape($ItemGroup['group_id'])."'"
						.($secure_access?' AND customers_id = "'.$db->escape($login->_customerID).'"':'')."
				");
	...
	function loadAllStore($secure_access = false)
	{
		...
		if($secure_access)
			$WHERE .= 'AND customers_id = "'.$db->escape($login->_customerID).'"';
	...
	function deleteBusiness($storeID)
	{
		global $db, $login;
		$output['ack'] = 'error';
		if($login->_customerInfo[0]['is_admin'] == 1)
	...
	function listBusinesses_HTML($cID = null, $Location = null, $Search = null, $limit = 100, $Page = 0 , $Mile_Pagination = 1)
	{
		...
		if(sizeof($Search) > 0)
		{
			if(isset($Search['Keyword']) && !empty($Search['Keyword']))
				$WHERE .= ' AND s.store_name like "%'.$db->escape($Search['Keyword']).'%"';
				
			if(isset($Search['State']) && !empty($Search['State']))
				$WHERE .= ' AND s.store_state COLLATE UTF8_GENERAL_CI LIKE "%'.$db->escape($Search['State']).'%"';
				
			if(isset($Search['City']) && !empty($Search['City']))
				$WHERE .= ' AND s.store_city like "%'.$db->escape($Search['City']).'%"';
				
			if(isset($Search['Zipcode']) && !empty($Search['Zipcode']))
				$WHERE .= ' AND s.store_zipcode like "%'.$db->escape($Search['Zipcode']).'%"';
			
			
			if(isset($Search['CityOrState']) && !empty($Search['CityOrState']))
				$WHERE .= ' AND (s.store_city like "%'.$db->escape($Search['CityOrState']).'%" or s.store_city like "%'.$db->escape($Search['CityOrState']).'%") ';
		...
		else if(!is_null($cID))
		{
			...
			$WHERE .= ' AND c2s.category_id = "'.$db->escape($cID).'"';
			...
			if(isset($Search['Keyword']) && !empty($Search['Keyword']))
			{
				$db->QRY("
					...
					INSERT INTO
						...
						VALUES
						(
							'".$db->escape($Search['Keyword'],false)."',
							0
						)
