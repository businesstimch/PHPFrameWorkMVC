<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$(document).on(touchOrClick,"#Delete_Order",function(){
			if($(".T1_List_Contents_Selected").length > 0)
				timconfirm('<i class="fa fa-trash"></i> Delete a Order','Are you sure?',function(){
					var oID = $(".T1_List_Contents_Selected").data("oid");
					
					$.ajax({
						type: "POST",
						url: "<?echo __AjaxURL__?>?ajaxProcess",
						data: "menu=deleteOrder&oID="+oID,
						success: function(d){
							toggelMenu.hide();
							var res = $.parseJSON(d);
							if(res.ack == 'success')
		...
		$(document).on(touchOrClick,"#Detail_Order",function(){
			if($(".T1_List_Contents_Selected").length > 0)
			{
				window.location = "/corp/sales/orders/detail/?oID="+$(".T1_List_Contents_Selected").data("oid");
			}
			else
...
<h2 id="PG_Title"><i class="fa fa-th-large"></i> Orders</h2>
<div id="PG_Menu">
	<div class="PdSelectedMenu button button_white" data-tooltip="Detail Order" id="Detail_Order" style="display:none;"><i class="fa fa-edit"></i></div>
	<div class="PdSelectedMenu button button_red" data-tooltip="Delete Order" id="Delete_Order" style="display:none;"><i class="fa fa-trash"></i></div>
	<div id="Refresh_BTN" class="button button_gray" data-tooltip="Refresh"><i class="fa fa-rotate-left"></i></div>
	<div id="Search_Block"><input type="text" id="Search_ProduT1_Keyword" value="<?php echo $Search_Keyword;?>" placeholder="Search by Order #, Name, Item SKU." /></div>
