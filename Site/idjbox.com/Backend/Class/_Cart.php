...
class _Cart extends GGoRok
{
				...
				$this->db->QRY("
					...
					VALUES
						(
							...
							'".$this->db->escape($itemDATA['storeID'])."',
							'".$this->db->escape($itemDATA['ItemID'])."',
							'".$this->db->escape($Item[0]['item_price'])."',
							'".$this->db->escape($itemDATA['ItemQTY'])."',
							'".$this->db->escape($Item[0]['item_name'])."',
							'".$this->db->escape(($Item[0]['item_texable'] == 1? $Item[0]['store_item_tax'] : 0))."',
							now()
						)
	...
	function findItem_IF_Exsist($itemDATA)
	{
		...
		$exist = $this->db->QRY("
			...
			FROM
				b_burugo_customers_cart
			WHERE
				item_id = '".$this->db->escape($itemDATA['ItemID'])."' AND
				customers_id = '".$this->db->escape($this->login->_customerID)."'
		");
	...
	function deleteItems_inCart($itemArrayID_inCart)
	{
		...
		if(is_array($itemArrayID_inCart))
			...
			$where .= ' cart_item_id = "'.$this->db->escape($itemArrayID_inCart).'" ';
		else
		{
			$output['ack'] = 'error';
			return $output;
	...
	function getItems_inCart($StoreID = null)
	{
		...
		$Items = $this->db->QRY("
			...
			WHERE
				c.customers_id = '".$this->db->escape($this->login->_customerID)."' AND
				c.store_id = '".$this->db->escape($StoreID)."'
			ORDER BY
		...
		if($customerID > 0 && $storeID > 0)
		{
			$this->db->QRY("
				DELETE FROM
					b_burugo_customers_cart
				WHERE
					customers_id = ".$this->db->escape($customerID)."
				AND store_id = ".$this->db->escape($storeID).";
			");
			$output['ack'] = 'success';
		}
		else
