<?
class _Subscription extends GGoRok
{
   ...
   function get_MethodList($wCoupon=false)
	...
	function get_PaymentSubscription($customerID = null, $storeID = null, $subsID = null)
	{
        ...
        if(!is_null($subsID))
			$WHERE .= ( $WHERE == '' ? ' WHERE ' : ' AND ' ).' Sub.subscription_id = '.$this->db->escape($subsID);
        /*
		if(!is_null($storeID))
			$WHERE .= ( $WHERE == '' ? ' WHERE ' : ' AND ' )." Sub.store_id = '".$this->db->escape($storeID)."' ";
	...
	function get_Subscription($subscriptionID = null)
	{
		...
		if(!is_null($subscriptionID))
			$WHERE .= ( $WHERE == '' ? ' WHERE ' : ' AND ' ).' H.subscription_id = '.$this->db->escape($subscriptionID);
		...
		$Output = $this->db->QRY($str_sql.$WHERE);
	...
	function get_SubscriptionHistoryList($customerID = null, $storeID = null, $subsID = null)
	{
		...
		if(!is_null($customerID))
			$WHERE .= ( $WHERE == '' ? ' WHERE ' : ' AND ' ).' H.customers_id = '.$this->db->escape($customerID);
			
		if(!is_null($storeID))
			$WHERE .= ( $WHERE == '' ? ' WHERE ' : ' AND ' ).' H.store_id = '.$this->db->escape($storeID);
		
		if(!is_null($subsID))
			$WHERE .= ( $WHERE == '' ? ' WHERE ' : ' AND ' ).' H.subscription_id = '.$this->db->escape($subsID);
	...
	function get_Package($package_id)
	{
		$str_sql = "
			...
			WHERE
				package_id = '".$this->db->escape($package_id)."' AND
				package_type = 0 AND
				package_group = 1
			LIMIT
				1
	...
	function update_Subscription($subsID, $packageID)
	{
		$str_sql = "
			...
			SET
				package_id = ".$this->db->escape(urldecode($packageID))."
			WHERE subscription_id = ".$this->db->escape(urldecode($storeID))."; 
		";
		
		$new_ID = $this->db->QRY($str_sql);
	...
	function update_ReferenceInfo($storeID, $referID=3)
	{
		...
		if($storeID > 0 )
		{
			$str_sql = "
				SELECT reference_id
				FROM b_burugo_refer
				WHERE store_id = ".$this->db->escape($storeID)."
			";
			
			$ret_sql = $this->db->QRY($str_sql);
			
			if(sizeof($ret_sql) > 0)
			{
				/*
				$str_sql = ' UPDATE b_burugo_refer
						SET customers_id = '.$this->db->escape($referID)
					.' WHERE reference_id = '.$this->db->escape($ret_sql[0]['reference_id']).';';
				$ret_sql = $this->db->QRY($str_sql);
				*/
				$output['ack'] = 'success';
			}
			else
			{
				$str_sql = "
					...
					VALUES (".$this->db->escape($referID).", ".$this->db->escape($storeID).", now() );
				";
	...
	function update_StorePaid($storeID, $exp_date)
	{
		...
		$str_sql = "
			...
			SET
				...
				subscription_expiration_date = '".$this->db->escape(urldecode($exp_date))."'
			WHERE store_id = ".$this->db->escape(urldecode($storeID))."; 
		";
		
		$new_ID = $this->db->QRY($str_sql, true);
	...
	function create_Subscription($customerID, $storeID, $package_id, $is_active = 0, $memo=null )
   {
		...
		$str_sql = "
			...
			VALUES
			(
				".$this->db->escape(urldecode($customerID)).",
				".$this->db->escape(urldecode($package_id)).",
				".$this->db->escape(urldecode($storeID)).",
				now(),
				".$this->db->escape(urldecode($is_active)).",
				'".$this->db->escape(urldecode($memo))."'
			)
		";
	...
	function create_SubscriptionHistory($subscription_id, $subscription_method_id, $customers_id, $store_id, $invoice_number, $payment_amt, $memo=null)
   {
		...
		$str_sql = "
			...
			VALUES
			(
				".$this->db->escape(urldecode($subscription_id)).",
				".$this->db->escape(urldecode($subscription_method_id)).",
				".$this->db->escape(urldecode($customers_id)).",
				".$this->db->escape(urldecode($store_id)).",
				'".$this->db->escape(urldecode($invoice_number))."',
				
				'".$this->db->escape(urldecode($payment_amt))."',
				now(),
				'".$this->db->escape(urldecode($memo))."'
			)
		";
	...
	function create_SubscriptionAuth($trans_id, $customers_id, $auth_id, $card_type, $card_4digit, $subs_h_id, $response_code, $response_reason_code, $memo=null)
	{
//return '::'.$trans_id. '::'.$auth_id.'::'.$card_type.'::'.$card_4digit.'::'.$response_code.'::'.$response_reason_code;
		$str_sql = "
			...
			VALUES
			(
				'".$this->db->escape(urldecode($trans_id))."',
				".$this->db->escape(urldecode($customers_id)).",
				'".$this->db->escape(urldecode($auth_id))."',
				'".$this->db->escape(urldecode($card_type))."',
				'".$this->db->escape(urldecode($card_4digit))."',
				
				".$this->db->escape(urldecode($subs_h_id)).",
				".$this->db->escape(urldecode($response_code)).",
				".$this->db->escape(urldecode($response_reason_code)).",
				'".$this->db->escape(urldecode($memo))."'
			)
		";
	...
	function create_SubscriptionSubscr($auth_id, $subs_id)
	{
		...
		$str_sql = "
			...
			(
				'".$this->db->escape(urldecode($auth_id))."',
				".$this->db->escape(urldecode($subs_id))."
			)
		";
	...
	function create_SubscriptionCheck($is_clear, $subs_h_id, $check_no=null)
	{
		...
		$str_sql = "
			...
			VALUES
			(
				".$this->db->escape(urldecode($is_clear)).",
				".$this->db->escape(urldecode($subs_h_id)).",
				'".$this->db->escape(urldecode($check_no))."'
			)
		";
	...
	function create_SubscriptionCoupon($coupon_id, $subs_h_id)
	{
		...
		$str_sql = "
			...
			VALUES
			(
				".$this->db->escape(urldecode($coupon_id)).",
				".$this->db->escape(urldecode($subs_h_id))."
			)
		";
	...
	function insert_PyamentReturn($customerID, $storeID, $paymentType, $svcExpDate, $subscriptionID, $invoiceNum, $transID, $payAmt, $cardType, $cardNum, $responseCode )
	{
