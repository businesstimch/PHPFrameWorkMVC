<style type="text/css">
	...
	.ItemGroup_One .IG_Item_One{width:100%;line-height:30px;height:30px;border-bottom:1px solid white;}
	...
	.Item_One .Item_Detail{padding:10px;width:100%;box-sizing:border-box;background-color:#f7f7f7;margin-top:5px;display:none;}
	...
	.Item_One .Item_Pic_Box img{width:100%;height:100%;}
	...
	#Submit_Box .Saving{background-color:gray!important;}
	...
	.I_G_deActivated{background-color:gray!important;}
	...
	#StoreDescLong_INP{height:500px;}
	.ItemCatBox{background-color:#edf5fc;}
	#GC_ServiceSetting{background-color:#ececec;}
	.GGoRokCheckBox{overflow:auto; width:100%;height:200px;border-radius:10px;line-height:30px;}
	.GGoRokCheckBox .GC_ChkBox_One{width:100%;padding-top:10px;padding-bottom:10px;border-bottom:1px dotted white;cursor:pointer;}
	.GGoRokCheckBox .GC_ChkBox_One:hover{background-color:#e0eefa;}
	.GGoRokCheckBox .GC_ChkBox{border:1px solid #aeaeae;width:28px;height:28px;background-color:white;border-radius:5px;margin-left:10px;margin-right:5px;text-align:center;}
	.GGoRokCheckBox .GC_ChkBox i{display:none;line-height:30px;color:#70b2e9;}
	.GGoRokCheckBox .GC_ChkBox_Checked i{display:block!important;}
	.GGoRokCheckBox .GC_ChkBox_Name{margin-left:5px;}
...
<script type="text/javascript" src="/Core/JS/tinymce/tinymce.min.js"></script>
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$(document).on(touchOrClick,'.GC_ChkBox_One',function(){
			var obj = $(this).find('.GC_ChkBox');
			if(obj.hasClass('GC_ChkBox_Checked'))
			{
				if(typeof $(this).data('value') != "undefined")
					$(this).data('value',-1);
				obj.removeClass('GC_ChkBox_Checked');
			}
			else
			{
				if(typeof $(this).data('value') != "undefined")
					$(this).data('value',1);
				obj.addClass('GC_ChkBox_Checked');
			}
					...
					$.ajax({
						...
						success: function(d){
							...
							if(res.ack == 'success')
							{
								...
								{
									tinymce.activeEditor.insertContent('<img src="/Template/'+res.imgs[0]+'" />');
	...
	var Submit = new function(){
		...
		this.submit = function(){
			...
			args['StoreDescLong_INP'] = tinyMCE.activeEditor.getContent({format : 'raw'});
			...
			$(".Item_One").each(function(){
				...
				args['Items'][i]['ID'] = $(this).data('itemid');
				args['Items'][i]['N'] = $(this).find('.Item_Name_Inp').val();
				args['Items'][i]['D'] = $(this).find('.Item_D_Inp').val();
				args['Items'][i]['P'] = $(this).find('.Item_Price_Inp').val();
				args['Items'][i]['ST'] = $(this).find('.Item_ItemStock_Inp').val();
				args['Items'][i]['ACT'] = ($(this).find('.I_Activate_BTN i').hasClass('fa-eye') ? 1 : 0 );
				args['Items'][i]['I2G'] = {};
				var j = 0;
				$(this).find('.GC_ChkBox_One').each(function(){
					if($(this).find('.GC_ChkBox_Checked').length)
					{
						args['Items'][i]['I2G'][j] = $(this).data('grpid');
						j++;
					}
				});
				
				
				if($(this).find('.Item_Pic_Inp') != "")
				{
					var ImgFile = $(this).find('.Item_Pic_Inp')[0];
					FormGroup.append('ItemImg_'+i,ImgFile.files[0]);
				}
	...
	var Items = new function(){
		...
		this.noGroupAdded_Text = '<div class="noGroup"><?php echo $this->_Lang_ceo_business['no_category_added'];?></div>';
		this.noItemAdded_Text = '<div class="noGroup"><?php echo $this->_Lang_ceo_business['add_item_to_sell'];?></div>';
		this.addGroup_HTML;
		this.addItem_HTML;
		...
		this.init = function(){
			...
			$.ajax({
				type: "POST",
				url: "/add?ajaxProcess",
				data: "menu=getAddItemAndGroup_HTML"+(typeof Store_ID != "undefined" ? "&cID="+Store_ID:""),
				success: function(d){
					var res = $.parseJSON(d);
					if(res.ack == 'success')
					{
			...
			$(document).on(touchOrClick,'.I_Up_BTN',function(){
				var MeObj = $(this).parents('.Item_One');
				var PrevObj = MeObj.prev('.Item_One');
				...
				if(PrevObj.length > 0)
				{
					...
					$(this).parents('.Item_One').replaceWith(PrevHTML);
					PrevObj.replaceWith(MeHTML);
			...
			$(document).on(touchOrClick,'.I_Down_BTN',function(){
				var MeObj = $(this).parents('.Item_One');
				var NextObj = MeObj.next('.Item_One');
				...
				if(NextObj.length > 0)
				{
					...
					$(this).parents('.Item_One').replaceWith(NextHTML);
					NextObj.replaceWith(MeHTML);
			...
			$(document).on(touchOrClick,'.IG_Activate_BTN',function(){
				...
				if(Obj.hasClass('fa-eye'))
				{
					...
					$(this).addClass("I_G_deActivated");
				}
				else
				{
					Obj.switchClass( "fa-eye-slash", "fa-eye", 1000, "easeInOutQuad" );
					$(this).removeClass("I_G_deActivated");
				}
			});
		...
		this.checkAndFill = function(){
			
			if($(".Item_One").length == 0)
			{
				$("#Item_Box").html(this.noItemAdded_Text);
			}
			
			if($(".ItemGroup_One").length == 0)
			{
				$("#ItemGroup_Box").html(this.noGroupAdded_Text);
			}
		};
	...
	var Editor = new function(){
		this.init = function(){
			tinymce.init({
				selector:'#StoreDescLong_INP',
				language : 'ko',

				toolbar: [
					'undo redo | styleselect | bold italic | alignleft aligncenter alignright alignjuestify | addimage',
...
<div class="Front_Contents_Body Cover">
	...
	<div class="Contents_Box_Wrap">
		...
		<div class="Contents_Box_One Glow">
			<div class="CBox_Title"><span>기타설정</span></div>
			<div class="CBox_Body">
				<div class="Box_Row">
					<div class="Box_Title">서비스 설정</div>
					<div class="Box_Field">
						<div class="ItemD_Fields GGoRokCheckBox" id="GC_ServiceSetting">
							<div class="GC_ChkBox_One Glow" data-submit="true" data-value="<?php echo (isset($Business['B']) ? $Business['B'][0]['Store_Service_Reservation'] : -1);?>" id="ReservationService_INP">
								<div class="GC_ChkBox<?php echo (isset($Business['B']) && $Business['B'][0]['Store_Service_Reservation'] == 1 ? " GC_ChkBox_Checked" : "");?>">
									<i class="fa fa-check"></i>
								</div>
								<div class="GC_ChkBox_Name"><i class="fa fa-calendar-check-o"></i> 예약기능</div>
							</div>
							<div class="GC_ChkBox_One Glow" data-submit="true" data-value="<?php echo (isset($Business['B']) ? $Business['B'][0]['Store_Service_Delivery'] : -1);?>" id="DeliveryService_INP">
								<div class="GC_ChkBox<?php echo (isset($Business['B']) && $Business['B'][0]['Store_Service_Delivery'] == 1 ? " GC_ChkBox_Checked" : "");?>">
									<i class="fa fa-check"></i>
								</div>
								<div class="GC_ChkBox_Name"><i class="fa fa-truck"></i> 배달기능</div>
							</div>
							<div class="GC_ChkBox_One Glow" data-submit="true" data-value="<?php echo (isset($Business['B']) ? $Business['B'][0]['Store_Service_Pickup'] : -1);?>" id="PickupService_INP">
								<div class="GC_ChkBox<?php echo (isset($Business['B']) && $Business['B'][0]['Store_Service_Pickup'] == 1 ? " GC_ChkBox_Checked" : "");?>">
									<i class="fa fa-check"></i>
								</div>
								<div class="GC_ChkBox_Name"><i class="fa fa-hand-grab-o"></i> 픽업기능</div>
							</div>
						</div>
						
					</div>
					<div class="Box_Desc">
						 예약, 배달, 픽업 기능 중 손님들에게 제공 하고자 하는 서비스를 선택해 주세요.
					</div>
				</div>
			</div>
		</div>
		...
		<div class="Contents_Box_One Glow">
			...
			<div class="CBox_Body">
				<div class="Box_Row">
					...
					<div class="Box_Field">
						<input type="text" value="<?php echo (isset($Business['B']) ? $Business['B'][0]['Store_ItemTitle'] : '');?>" placeholder="<?php echo $this->_Lang_ceo_business['item_name_ex'];?>" id="ItemTitle_INP" data-submit="true" />
					</div>
					<div class="Box_Desc">
				...
				<div class="Box_Row">
					...
					<div class="Box_Field">
						...
						<div id="ItemGroup_Box" class="ItmAndGrp_Box"><?php echo (isset($ItemGroup_HTML) ? $ItemGroup_HTML : '');?></div>
					</div>
				</div>
				...
				<div class="Box_Row">
					...
					<div class="Box_Field">
						...
						<div id="Item_Box" class="ItmAndGrp_Box"><?php echo (isset($Items_HTML) ? $Items_HTML : '');?></div>
					</div>
				</div>
			</div>
		</div>
