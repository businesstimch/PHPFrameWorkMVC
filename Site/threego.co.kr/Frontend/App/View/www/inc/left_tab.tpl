<script type="text/javascript">
	...
	var Cart = new function(){
		...
		this.init = function(){
			self.refreshCart();
			$(document).on(touchOrClick,".Item_AddCart",function(){
				var args = {};
				
				args['cID'] = cID;
				args['Item_ID'] = $(this).parents('.Item_One').data('itemid');
				args['QTY'] = '1';
				self.addCart(args);
			});
		};
		
		this.addCart = function(args){
			
			timconfirm("카트","장바구니에 추가 할까요?",function(){
				$.ajax({
					type: "POST",
					url: "/global/www/Cart?ajaxProcess",
					data: 'menu=Add&ItemID='+args['Item_ID']+"&cID="+args['cID']+"&QTY="+args['QTY'],
					success: function(d){
						var res = $.parseJSON(d);
						if(res.ack == 'success')
						{
							showSideMSGBox("<i class='fa fa-shopping-basket'></i> 장바구니에 추가 되었습니다.","msgBox_One_1");
							self.refreshCart();
							TopMenu.OpenDialog($("#ShoppingCart_Dialog"));
						}
		...
		this.refreshCart = function()
		{
			$.ajax({
				...
				success: function(d){
					...
					if(res.ack == 'success')
					{
						$("#CartItem_Block").html(res.html);
						if(typeof FlyingCart != "undefined")
							FlyingCart.refreshCart();
...
<style type="text/css">
	...
	#ShoppingCart_Dialog{left:105px;width:500px;}
	#ShoppingCart_Dialog .LD_Arrow{left:158px;}
	#ShoppingCart_Dialog #CartItem_Block{width:100%;min-height:200px;max-height:400px;overflow:auto;}
	#ShoppingCart_Dialog .CenterMsg{line-height:200px;}
	...
	.CartItem_One{width:100%;line-height:40px;color:white;padding-top:5px;padding-bottom:5px;border-bottom:1px dotted gray;}
	.CartItem_One:hover{background-color:#454545;}
	.CartItem_One .CI_Img{overflow:hidden;margin-left:10px;width:40px;height:40px;border-radius:5px;margin-right:10px;}
	.CartItem_One .CI_Img img{width:100%;height:100%;}
	.CartItem_One .CI_Name{width:200px;}
	.CartItem_One .CI_Qty{width:50px;}
	.CartItem_One .CI_Price{width:100px;}
	.CartItem_One .CI_Menu{width:70px;}
	.CartItem_One .CIM_One{width:30px;height:30px;text-align:center;line-height:30px;cursor:pointer;margin-top:5px;border-radius:5px;margin-right:5px;color:white;}
	
	.CartItem_One .CIM_Detail{background-color:#43a6d4;}
	.CartItem_One .CIM_Delete{background-color:#e75252;}
	.CartItem_One .CIM_Delete:hover{background-color:#ef6b6b;}
	
	.PPC_Item_One{width:700px;line-height:40px;color:white;padding-top:5px;padding-bottom:5px;border-bottom:1px dotted white;background-color:#43a6d4;clear:both;}
	.PPC_Item_One:hover{background-color:#454545;}
	.PPC_Item_One .CI_Img{overflow:hidden;margin-left:10px;width:40px;height:40px;border-radius:5px;margin-right:10px;}
	.PPC_Item_One .CI_Img img{width:100%;height:100%;}
	.PPC_Item_One .CI_Name{width:400px;}
	.PPC_Item_One .CI_Qty{width:50px;}
	.PPC_Item_One .CI_Price{width:100px;}
	.PPC_Item_One .CI_Menu{width:70px;}
	.PPC_Item_One .CIM_One{width:30px;height:30px;text-align:center;line-height:30px;cursor:pointer;margin-top:5px;border-radius:5px;margin-right:5px;color:white;}
	.PPC_Item_One .CIM_Detail{background-color:#43a6d4;}
	.PPC_Item_One .CIM_Delete{background-color:#e75252;}
	.PPC_Item_One .CIM_Delete:hover{background-color:#ef6b6b;}
	
	
	
	.CenterMsg{width:100%;text-align:center;color:white;}
...
<div id="Top_Menu">
	...
	<div id="ShoppingCart_BTN" data-dialogid="ShoppingCart_Dialog" data-tooltip="<?php echo $this->_Lang_www_inc_lefttab['shoppingcart'];?>" class="Top_Menu_One Glow"><i class="fa fa-shopping-basket"></i></div>
...
<div id="Left_Tab">
	...
	<div id="ShoppingCart_Dialog" class="Top_Dialog">
		<div class="LD_Arrow"></div>
		<div class="Close_Dialog"><i class="fa fa-close"></i></div>
		<div id="CartItem_Block">
			<?php
			if($this->login->isLogIn())
			{
				
				$CartItems = $this->_Model_Cart->Get(array(
				...
				echo '<div class="CenterMsg">장바구니에 추가된 상품 / 아이템이 없습니다.</div>';
